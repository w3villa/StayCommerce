...
module Stay
  class Taxon < ApplicationRecord
    ...
    belongs_to :taxonomy, class_name: 'Stay::Taxonomy', inverse_of: :taxons
    has_many :classifications, -> { order(:position) }, dependent: :delete_all, inverse_of: :taxon
    has_many :properties, through: :classifications
    ...
    has_one :icon, as: :viewable, dependent: :destroy, class_name: 'Stay::TaxonImage'

    scope :for_store, ->(store) { joins(:taxonomy).where(stay_taxonomies: { store_id: store.id }) }

    self.whitelisted_ransackable_associations = %w[taxonomy]
    self.whitelisted_ransackable_attributes = %w[name permalink]

    scope :for_stores, ->(stores) { joins(:taxonomy).where(stay_taxonomies: { store_id: stores.ids }) }
