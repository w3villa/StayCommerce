    ...
    class TokensController < Devise.api.config.base_controller.constantize
      ...
      def sign_up
        ...
        ActiveRecord::Base.transaction do
          ...
          if service.success?
            ...
            Stay::UserMailer.new_host_signup(token.resource_owner).deliver_now if token.resource_owner.persisted? && token.resource_owner.stay_host?
          ...
          error_message = error_response.body[:error_description].is_a?(Array) ?
                          error_response.body[:error_description].join :
                          error_response.body[:error_description].to_s
      ...
      def sign_in
        ...
        if service.success?
          ...
          return render json: { message: "login Successfully", access_token: token_response&.token.access_token, success: true }, status: token_response.status
        end
      ...
      def revoke
        ...
        if service.success?
          ...
          return render json: { message: "Logout Successfully", success: true }, status: :ok
        end
